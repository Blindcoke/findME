    ...
    const fetchCaptives = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/captives/?status=deceased|reunited`,
          { credentials: "include" }
        );
              ...
              <CardContent className="p-6 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {captive.status === 'deceased' ? (
                    <span className="px-3 py-1 bg-gradient-to-r from-red-500/80 to-rose-600/80 rounded-full text-sm font-medium text-white shadow-md">
                      Загиблий
                    </span>
                  ) : (
                    <span className="px-3 py-1 bg-gradient-to-r from-indigo-500/80 to-yellow-600/80 rounded-full text-sm font-medium text-white shadow-md">
                      Возз’єднаний
                    </span>
                  )}
