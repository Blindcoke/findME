...
interface SearchingPersonsProps {
  isAuthenticated: boolean;
}

const SearchingPersons: React.FC<SearchingPersonsProps> = ({ }) => {
  ...
  useEffect(() => {
    const fetchCaptives = async () => {
      try {
        ...
        setCaptives(data); // Always update captives even if empty
        setIsLoading(false);
      } catch (error) {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="bg-emerald-900/30 backdrop-blur-lg rounded-2xl p-6 shadow-xl mb-8">
        <div className="flex flex-col md:flex-row gap-4 items-center mb-6">
          ...
          />
          <Link to="/searching/add" className="w-full md:w-auto">
              ...
              <CardContent className="p-6 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <span className="px-3 py-1 bg-gradient-to-r from-green-500/80 to-emerald-600/80 rounded-full text-sm font-medium text-white shadow-md">
                    {captive.status === 'searching' ? 'Розшукується' : ''}
                  </span>
...
export default SearchingPersons;